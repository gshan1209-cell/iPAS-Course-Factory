...
Card-face source format is intentionally compact:

`指引:「<主題名稱>」；考題:<年份-梯次>、<年份-梯次>…`

Only exam sessions that truly contain mapped questions for the current card may be displayed.

Do **not** display on the card face:

- question numbers such as `Q14`;
- question counts;
- level / subject text after `考題:`;
- exam-rate percentages.
...
The data layer still retains the full source evidence, including exact question numbers, level / subject, official URLs or Drive mirrors, and audit references.
...
- `EVIDENCE_TEXT_LOCKED = true` — meaning the deterministic source footer exactly matches the governed compact citation;
...
- footer displays a question number;
- footer displays a question count;
- footer displays a session that has no mapped question evidence;
- footer repeats the same exam session;
